 ...
 *
 * Plugin to convert string locations to SimpleGeo IDs and vice versa
 *
 * PHP version 5
 *
 ...
 * @seeAlso  Location
 */
class SimpleGeoPlugin extends Plugin
{
    const LOCATION_NS = 1;
    ...
    function onLocationFromName($name, $language, &$location)
    {
        ...
        try {
            $geonames = $this->getSimpleGeo('search',
                                           array('maxRows' => 1,
                                                 'q' => $name,
                                                 'lang' => $language,
                                                 'type' => 'xml'));
    ...
    function onLocationFromId($id, $ns, $language, &$location)
    {
        ...
        try {
            $geonames = $this->getSimpleGeo('hierarchy',
                                           array('geonameId' => $id,
                                                 'lang' => $language));
        } catch (Exception $e) {
    ...
    function onLocationFromLatLon($lat, $lon, $language, &$location)
    {
        ...
        try {
          $geonames = $this->getSimpleGeo('findNearbyPlaceName',
                                         array('lat' => $lat,
                                               'lng' => $lon,
                                               'lang' => $language));
        } catch (Exception $e) {
    ...
    function getSimpleGeo($method, $params)
    {
    ...
    {
        $versions[] = array('name' => 'SimpleGeo',
                            'version' => STATUSNET_VERSION,
                            'author' => 'Evan Prodromou',
                            'homepage' => 'http://status.net/wiki/Plugin:SimpleGeo',
                            'rawdescription' =>
                            _m('Uses <a href="http://geonames.org/">SimpleGeo</a> service to get human-readable '.
